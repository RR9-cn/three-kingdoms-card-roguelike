import {previewForge,randomState,type ForgeView,type ForgeState,type ForgeAction,type ForgeTuning} from '@three-card/core';
import {COMPANIONS,FORGE_RULES as R,type CompanionId} from '@three-card/content';
export function bestForgeHand(v:ForgeView,tuning?:ForgeTuning){const s:ForgeState={...v,hand:v.hand.map(c=>c.id),rng:randomState('public-view'),draw:[],discard:[]};let best={ids:[] as string[],score:-1,category:0};for(let i=0;i<v.hand.length;i++)for(let j=i+1;j<v.hand.length;j++)for(let k=j+1;k<v.hand.length;k++){const ids=[v.hand[i].id,v.hand[j].id,v.hand[k].id];const p=previewForge(s,ids,tuning);if(p.total>best.score)best={ids,score:p.total,category:p.category};}return best;}
...
export function forgeStrategy(v:ForgeView,starter:CompanionId,tuning?:ForgeTuning):Omit<ForgeAction,'seq'>{const pref=preferences[starter]??preferences.guanyu,rank=(id:CompanionId)=>pref.includes(id)?pref.indexOf(id):99;const sorted=(ids:CompanionId[])=>[...ids].sort((a,b)=>rank(a)-rank(b));
 if(v.phase==='starter')return{type:'starter',id:starter};
 if(v.phase==='prepare')return{type:'begin'};
 if(v.phase==='battle')return{type:'play',ids:bestForgeHand(v,tuning).ids};
 ...
 if(v.phase==='shop'){
  if(v.pendingEdit&&v.gold>=R.editCost)return{type:'buy-edit'};
