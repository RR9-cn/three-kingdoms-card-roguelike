...
export function counterplayStrategy(v:ForgeView,starter:CompanionId):Omit<ForgeAction,'seq'>{
 if(v.phase!=='battle')return forgeStrategy(v,starter);
 const best=bestForgeHand(v),gap=v.target-v.score;
