...
async function act(a:Omit<ForgeAction,'seq'>){if((a.type==='play'||a.type==='discard')){for(const id of a.ids!)await click('card:'+id);await click(a.type);}else if(a.type==='begin')await click('begin:normal');else if(a.type==='recruit'||a.type==='buy'){await click(a.type+':'+a.id);if(a.replace)await click('replace:'+a.replace);}else{if(a.type==='bank'&&await page.locator('[data-action=skip-animation]').count())await click('skip-animation');await click(a.type);}}
try{
 ...
 await writeFile('docs/evidence/forge-v07-desktop-smoke.json',JSON.stringify({version:'0.7.0',saveContentVersion:'0.7.0',schemaVersion:8,platform:process.platform,arch:process.arch,packaged:!!process.env.THREE_CARD_EXECUTABLE,offlineLocalPages:true,legacyNavigation:true,rendererNodeDisabled:true,minimizePause:true,restartRestore:true,fullCampaignVictory:true,endlessContinuation:true,wagerSettledAndRestored:true,randomPostBattleEdit:true,simpleScoring:true,zhangJiaoAnimation:true,errors},null,2)+'\n');console.log('Forge desktop: simple scoring, native campaign, Zhang Jiao animation, random edit, local legacy navigation, pause and restart restore passed.');
