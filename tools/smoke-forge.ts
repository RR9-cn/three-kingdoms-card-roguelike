...
try{
 ...
 const completed=await read();assert.equal(typeof completed.momentum,'number');assert.equal(typeof completed.primed,'boolean');await click('extend');
 ...
 await writeFile('docs/evidence/forge-v06-desktop-smoke.json',JSON.stringify({version:'0.6.0',saveContentVersion:'0.6.0',schemaVersion:7,platform:process.platform,arch:process.arch,packaged:!!process.env.THREE_CARD_EXECUTABLE,offlineLocalPages:true,legacyNavigation:true,rendererNodeDisabled:true,minimizePause:true,restartRestore:true,fullCampaignVictory:true,endlessContinuation:true,wagerSettledAndRestored:true,randomPostBattleEdit:true,zhangJiaoAnimation:true,controlledPursuitState:true,errors},null,2)+'\n');console.log('Forge desktop: native campaign, controlled pursuit state, Zhang Jiao animation, random edit, local legacy navigation, pause and restart restore passed.');
