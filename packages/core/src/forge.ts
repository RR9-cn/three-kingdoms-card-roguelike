...
export interface ForgeState {
 schemaVersion:6;contentVersion:typeof FORGE_VERSION;seed:string;seq:number;rng:RandomState;phase:ForgePhase;stage:number;
...
export function isForgeState(value:unknown):value is ForgeState{try{
 ...
 if(s.schemaVersion!==6||s.contentVersion!==FORGE_VERSION||typeof s.seed!=='string'||!int(s.seq)||!int(s.stage)||s.stage>=STAGES.length)return false;
