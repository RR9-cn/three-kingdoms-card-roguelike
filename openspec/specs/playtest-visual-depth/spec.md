# playtest-visual-depth Specification

## Purpose
Define the presentation-layer depth of the midnight-auction playtest client: layered card material under one light direction, CSS-3D card depth with pointer-following tilt, selection lift tied to the slot order, a step-bound hammer-fall settlement transition, motion degradation, and the presentation contracts the change must preserve (rules kernel, save protocol, CSP, accessibility, verification channels).
