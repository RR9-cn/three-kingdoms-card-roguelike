...
Whenever a card with rank 4 or lower scores, Liu Bei SHALL add exactly 6 multiplier and SHALL NOT add points.

#### Scenario: A low-rank card scores normally
- **WHEN** a card with rank 4 or lower scores while Liu Bei is owned
- **THEN** the current multiplier increases by 6 and the card receives no points from Liu Bei
...
- **THEN** Liu Bei adds 6 multiplier for that additional scoring event without adding points
