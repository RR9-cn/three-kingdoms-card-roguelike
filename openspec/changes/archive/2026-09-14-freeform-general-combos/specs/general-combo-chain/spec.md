...
The system SHALL allow pair, shared-suit, straight, and rightmost-card generals to each add one deterministic additional scoring event for their selected card, and SHALL preserve every event when multiple sources target the same card.
