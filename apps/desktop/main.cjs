...
app.whenReady().then(() => {
  const win = new BrowserWindow({ width:1440,height:900,minWidth:1000,minHeight:700,backgroundColor:'#111e20',title:'午夜落槌 · 藏品连锁',webPreferences:{preload:path.join(__dirname,'preload.cjs'),nodeIntegration:false,contextIsolation:true,sandbox:true,webSecurity:true} });
  ...
  Menu.setApplicationMenu(Menu.buildFromTemplate([{label:'午夜落槌',submenu:[{role:'about'},{type:'separator'},{role:'quit'}]},{label:'窗口',submenu:[{role:'togglefullscreen'},{role:'minimize'}]}]));
