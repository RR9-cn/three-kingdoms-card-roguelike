...
Steam PC 单人买断方向的三国卡牌肉鸽。当前默认是 **v0.7 简明计分版**：六张选三张，基础攻势始终是“三张点数之和 × 阵型倍率”，再由武将、军宝、成长和随机整编推动爆发。

先试「启程」→选关羽、周瑜或刘备→普通进军。每关胜后征募、整编、逛商店。支持1–6选牌、Enter出牌/下一手、自动存档与原生最小化暂停。

只想检查新版连锁时，点击首页“连锁试玩 · 直达张角”。选择含对子的三张牌，可以观察张飞重触发、追击令35%追加与连营鼓倍率增长。试玩退出后恢复原征程，不写入正式存档。
...
22个组件、5个槽、6种阵型、4类随机正向整编、8关、开局选核心、逐步结算、即时预览、刷新/买卖/替换。阵型倍率依次为×1、×2、×3、×4、×6、×10；升级只增加倍率。每场胜利后随机获得复制阵牌、升点、强化或牌型升级中的一项，也可跳过换3军资。默认随机种子，可输入 `forge-v051-7` 重玩完整征程测试种子；首页试玩使用独立的固定局面，不会暗中改变正式征程。

这是可玩开发原型，尚未验证真人重复游玩的吸引力。本轮保留四档连锁反馈，不增加额外战斗资源；第8关张角继续使用 PixelLab 待机、受击、雷法变招与败亡动作。张宝和其他角色仍使用代码图形。Windows原生、五人试玩、完整正式美术、Steam商店与上传均未完成。
...
本版保留追击令／连营鼓、得分提升稀有招募概率、结算后押分、单手纪录及通关后极限挑战，同时移除阵型基础兵力与组件顺序计分。旧存档保留，新规则需新征程。
...
连锁反馈验证：[v0.6 纯随机追击](docs/evidence/CHAIN_FEEDBACK_V06.md)。

简明计分验证：[v0.7 三张点数 × 阵型倍率](docs/evidence/SIMPLE_SCORING_V07.md)。
