...
test('Zhang Jiao uses distinct idle, hit, thunder and defeat pixel motions',async({page})=>{
 ...
 for(const id of idle.hand.slice(0,3))await page.locator(`[data-action="card:${id}"]`).click();await page.locator('[data-action=play]').click();await expect(page.locator('[data-boss-motion=hit]')).toBeVisible();await expect(page.locator('.boss-sheltered')).toContainText('谋士避雷！本手攻势未被削减');await page.waitForTimeout(220);await expect(page.locator('[data-boss-motion=hit]')).toHaveAttribute('src',/hit-[1-5]\.png/);await page.screenshot({path:'docs/evidence/boss-zhangjiao-hit.png'});
 await load(page,idle);for(const id of ['spear-1','cavalry-2','scheme-4'])await page.locator(`[data-action="card:${id}"]`).click();await page.locator('[data-action=play]').click();await expect(page.locator('.boss-punished')).toContainText('天雷命中！本手最终攻势 ×0.6');await expect(page.locator('[data-boss-motion=thunder]')).toBeVisible();await expect(page.locator('.ledger')).toContainText('未留下谋牌：最终攻势 ×0.6');await page.screenshot({path:'docs/evidence/boss-zhangjiao-struck.png'});
