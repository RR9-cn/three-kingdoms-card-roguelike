...
test('Zhang Jiao uses distinct idle, hit, thunder and defeat pixel motions',async({page})=>{
 ...
 let hit=forgeAction(idle,{seq:idle.seq,type:'play',ids:idle.hand.slice(0,3)}).state;await load(page,hit);await expect(page.locator('[data-boss-motion=hit]')).toBeVisible();await expect(page.locator('.boss-sheltered')).toContainText('谋士避雷！本手攻势未被削减');await page.waitForTimeout(220);await expect(page.locator('[data-boss-motion=hit]')).toHaveAttribute('src',/hit-[1-5]\.png/);await page.screenshot({path:'docs/evidence/boss-zhangjiao-hit.png'});
 let struck=forgeAction(idle,{seq:idle.seq,type:'play',ids:['spear-1','cavalry-2','scheme-4']}).state;await load(page,struck);await expect(page.locator('.boss-punished')).toContainText('天雷命中！本手最终攻势 ×0.6');await expect(page.locator('[data-boss-motion=thunder]')).toBeVisible();await expect(page.locator('.ledger')).toContainText('未留下谋牌：最终攻势 ×0.6');await page.screenshot({path:'docs/evidence/boss-zhangjiao-struck.png'});
...
test('home demo enters a disposable built Zhang Jiao battle',async({page})=>{
 ...
 await page.getByRole('button',{name:'规则',exact:true}).click();await expect(page.locator('.formation-guide-row')).toHaveCount(6);await expect(page.locator('.formation-guide')).toContainText('同袍连阵');await expect(page.locator('.formation-guide')).toContainText('45兵力 × 4倍率');await page.screenshot({path:'docs/evidence/formation-rules.png'});await page.waitForTimeout(180);await page.locator('[data-action=close]').click();await page.waitForTimeout(180);
 ...
 await expect(page.locator('.demo-strip')).toContainText('追击令35%追加');await expect(page.locator('[aria-label="张角首领战"]')).toBeVisible();await expect(page.locator('.band-card')).toHaveCount(5);await expect(page.locator('[data-boss-motion=idle]')).toBeVisible();await expect(page.locator('[data-action=prime]')).toHaveCount(0);await expect(page.locator('.momentum-panel')).toHaveCount(0);
 for(const id of ['spear-7','cavalry-7','bow-9'])await page.locator(`[data-action="card:${id}"]`).click();await expect(page.locator('.formation-definition')).toContainText('其中两张点数相同');await page.locator('[data-action=play]').click();await page.waitForTimeout(180);await page.locator('[data-action=skip-animation]').click();await expect(page.locator('.chain-tier.tier-0')).toContainText('未响 · 本手未追加');await page.screenshot({path:'docs/evidence/chain-feedback.png'});
