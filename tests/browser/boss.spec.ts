...
test('home demo enters a disposable built Zhang Jiao battle',async({page})=>{
 ...
 await expect(page.locator('.demo-strip')).toContainText('想看天雷惩罚');await expect(page.locator('[aria-label="张角首领战"]')).toBeVisible();await expect(page.locator('.band-card')).toHaveCount(5);await expect(page.locator('[data-boss-motion=idle]')).toBeVisible();
