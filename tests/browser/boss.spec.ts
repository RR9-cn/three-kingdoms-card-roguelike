...
test('home demo enters a disposable built Zhang Jiao battle',async({page})=>{
 ...
 for(const id of ['spear-7','cavalry-7','bow-9'])await page.locator(`[data-action="card:${id}"]`).click();await page.locator('[data-action=play]').click();await page.waitForTimeout(180);await page.locator('[data-action=skip-animation]').click();await expect(page.locator('.chain-tier.tier-0')).toContainText('未响 · 本手未追加');await page.screenshot({path:'docs/evidence/chain-feedback.png'});
