...
test('older build saves are preserved and never loaded under new targets',()=>{const map=new Map([['three-card:forge:v6:0','old-payload']]);const storage={get:(k:string)=>map.get(k)??null,set:(k:string,v:string)=>{map.set(k,v);}};assert.equal(loadForge(storage).state,null);assert.ok(loadForge(storage).notice);saveForge(storage,newForge('new'));assert.equal(map.get('three-card:forge:v6:0'),'old-payload');assert.equal(loadForge(storage).state?.contentVersion,'0.6.0');});
