...
test('seal damage increase is lower than pressure target increase',()=>{let s=battle(0);s.companions=[{id:'seal',growth:0}];const ids=s.hand.slice(0,3),plain=previewForge(s,ids);s.pressed=true;const pressed=previewForge(s,ids);assert.equal(pressed.mult,plain.mult*1.25);s.phase='prepare';s=act(s,{type:'begin',pressed:true});assert.equal(s.target,150);assert.ok(pressed.total/150<plain.total/100);});
