...
test('flank checks physical hand order, changes penalty with leading suit',()=>{const s=battle(6),ids=s.hand.slice(0,3);s.lastSuit=s.army.find(c=>c.id===ids[0])!.suit;assert.equal(previewForge(s,[...ids].reverse()).penalty,.5);s.lastSuit=s.lastSuit==='bow'?'spear':'bow';const p=previewForge(s,ids);assert.equal(p.penalty,1);const r=act(s,{type:'play',ids});assert.equal(r.result?.total,p.total);assert.equal(r.lastSuit,p.cards[0].suit);});
test('seal damage increase is lower than pressure target increase',()=>{let s=battle(0);s.companions=[{id:'seal',growth:0}];const ids=s.hand.slice(0,3),plain=previewForge(s,ids);s.pressed=true;const pressed=previewForge(s,ids);assert.equal(pressed.mult,plain.mult*1.25);s.phase='prepare';s=act(s,{type:'begin',pressed:true});assert.equal(s.target,210);assert.ok(pressed.total/210<plain.total/140);});

test('older build saves are preserved and never loaded under new targets',()=>{const map=new Map([['three-card:forge:v5:0','old-payload']]);const storage={get:(k:string)=>map.get(k)??null,set:(k:string,v:string)=>{map.set(k,v);}};assert.equal(loadForge(storage).state,null);assert.ok(loadForge(storage).notice);saveForge(storage,newForge('new'));assert.equal(map.get('three-card:forge:v5:0'),'old-payload');assert.equal(loadForge(storage).state?.contentVersion,'0.5.1');});
